    ...
    if ($result->num_rows > 0) {
        $paciente = $result->fetch_assoc();
         ...
         if ($resultHistoria->num_rows > 0) {
            $historiaClinica = $resultHistoria->fetch_assoc();
            ...
            class PDF extends FPDF{
                ...
                function PatientRecord($data , $historiaClinica)
                {
                    ...
                    $this->SetFont('Arial', '', 12);
                    $this->MultiCell(0, 10, mb_convert_encoding($historiaClinica['contenido'], 'UTF-8'));
                }
            }
            // Crear una instancia de la clase PDF
        $pdf = new PDF();
        $pdf->AddPage();
        $pdf->PatientRecord($paciente,$historiaClinica);
        ...
        $pdf->Output($nombreArchivo, 'D'); // Descargar el PDF con el nombre del paciente
            ...
            echo "Error al generar el PDF: No se encontró historia clínica para el paciente.";
        }        
    } else {
