...
function insertarDoctor($conn, $nombre, $apellido, $cargo, $matricula) {
    $sql = "INSERT INTO personal (nombre, apellido, cargo, matricula, tipo) VALUES (?, ?, ?, ?, 'medico')";
    $stmt = $conn->prepare($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["nombre"]) && isset($_POST["apellido"]) && isset($_POST["cargo"]) && isset($_POST["matricula"])) {
    ...
    if (validarCampos($nombre, $apellido, $cargo, $matricula)) {
        ...
        if (verificarMatriculaExistente($conn, $matricula)) {
            ...
            if (insertarDoctor($conn, $nombre, $apellido, $cargo, $matricula)) {
                echo "Registro exitoso";
                header("Location: doctores.php");
                exit();
            } else {
                echo "Error en el registro: " . $conn->error;
                echo '<a href="doctores.php">Volver a intentar</a>';
            }
        }
    }
}
